...
contains
  ...
  subroutine vmodel_user( io_prm, i0, i1, j0, j1, k0, k1, xc, yc, zc, vcut, rho, lam, mu, Qp, Qs, bd )
    ...
    !real(SP) :: Model_p(76,61,27), Model_s(76,61,27), x(76), y(61), z(27)
    real(SP) :: Model(96,82,78), x(96), y(82), z(78)    
    character(len=60) :: topo_dir
    real(SP) :: topo_user
    integer :: line, error
    integer :: nx, ny
    !! ----
    ...
    !open(2021, file=topo_dir, status = 'OLD', form = 'formatted',&
    ...
    do j = j0, j1
      do i = i0, i1
        ...
        !line = (j + 2) * (nx + 6) + (i + 3)
        !read(2021, fmt="(F10.3)", rec=line, IOSTAT=error)topo_user
        !bd(i,j,0) = topo_user
        bd(i,j,0) = topo0
        do k = k0, k1
          ...
          if( zs( k ) > bd(i,j,0) ) then
            ...
            call geomap__c2g( xc(i), yc(j), clon, clat, phi, new_lon, new_lat)
            !call find_location(Model_p, x, y, z, new_lon, new_lat, zc(k), vp0)
            !call find_location(Model_s, x, y, z, new_lon, new_lat, zc(k), vs0)
            call find_location(Model, x, y, z, new_lon, new_lat, zc(k), vp0) 
            vs0 = vp0 / sqrt(3.0)
            !! elastic medium
            vp1 = Cv(k) * vp0
            vs1 = Cv(k) * vs0
