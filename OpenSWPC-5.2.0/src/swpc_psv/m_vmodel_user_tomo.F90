...
contains
  ...
  subroutine vmodel_user( io_prm, i0, i1, k0, k1, xc, zc, vcut, rho, lam, mu, Qp, Qs, bd )
    ...
    real(SP) :: new_lon, new_lat    
    real(SP) :: phi, clon, clat
    !! ----
    ...
    do i = i0, i1
      ...
      do k = k0, k1
        ...
        if( zs( k ) > bd(i,0) ) then
          ...
          call geomap__c2g( xc(i), 0.0, clon, clat, phi, new_lon, new_lat)
          call find_location(Model_p, x, y, z, new_lon, new_lat, zc(k), Vp)
          call find_location(Model_s, x, y, z, new_lon, new_lat, zc(k), Vs)
